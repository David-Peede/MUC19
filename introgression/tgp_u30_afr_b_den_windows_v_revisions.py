...
# Define a function to compute U_{AFR,B,DEN}(1%, 30%, 100%) statistics in non-overlapping windows.
def tgp_U30_windows(chromosome, window_size):
    ...
    qc_windows_df = pd.read_csv(f'../windowing/tgp_den_masked_no_aa/{window_size}kb_nonoverlapping_variant_windows.csv.gz')
    ...
    U_AB_den_mat = np.empty((n_windows, 19))
    # For every window.
    for wind in range(n_windows):
        ...
        U_AB_den_mat[wind, :] = np.array(U_AB_den_list)
    # Export the the results matrix.
    np.savetxt(
        f'../muc19_results/tgp_den_masked_no_aa/u30_afr_b_den_chr{chromosome}_{window_size}kb.txt.gz',
        U_AB_den_mat, fmt='%d',
    )
    return
