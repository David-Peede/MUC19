...
# Define function to filter the sgdp joint vcf file.
def sgdp_all_archaics_merged_raw_vcf(vcf, chrom):
    """
    ###########################################################################
    INPUT
    ...
    qc_file = open('./vcf_bookkeeping/sgdp_all_archaics_merged_raw_qc/chr{0}_qc_report.txt'.format(chrom), 'w')
    ...
    with gzip.open(vcf, 'rt') as data:
        ...
        for line in data:
            ...
            # Else...
                ...
                # Grab the genotype info for the first individual in the sgdp.
                sgdp_gt = spline[9]
                ...
                # Else...
                    ...
                    # If the sgdp has an allele call...
                    if (sgdp_gt[0] != '.'):
                    ...
                    # Else...
                        ...
                        # Make all of the sgdp individuals homozygous for the reference allele since
                        # the sgdp data is imputed.
                        new_spline = [geno.replace('./.:.:.:.:.:.:.:.', '0|0:.:.:.:.:.:.:.') for geno in spline]
                        # Update the archaic site information.
                        new_spline[-4] = arc_gt_list[0]
                        new_spline[-3] = arc_gt_list[1]
                        new_spline[-2] = arc_gt_list[2]
                        new_spline[-1] = arc_gt_list[3]
                        # Write it to the new vcf.
...
sgdp_all_archaics_merged_raw_vcf(vcf=str(sys.argv[1]), chrom=str(sys.argv[2]))