...
def process_merged_line(line, chrom, anc_seq, dup_set):
    ...
    # Else-if the archaic genotype is missing.
        ...
        # Return the qc information.
        return '\t'.join([str(chrom), str(pos), '2', 'archaic_missing_gt'])+'\n', None, None
    # Else-if the archaic genotype quality is less than 40.
    elif int(arc_gt.split(':')[-1]) < 40:
        # Return the qc information.
        return '\t'.join([str(chrom), str(pos), '3', 'archaic_low_gq'])+'\n', None, None
    ...
    # Else-if the reference allele is the ancestral allele and there is no alternative allele.
        ...
        return None, '\t'.join([str(chrom), str(pos), '0', '0', 'all_anc_ref_anc'])+'\n', None
    # Else-if the reference allele is not the ancestral allele and there is no alternative allele.
    elif (anc_allele.upper() != ref) & (alt == '.'):
        # Return the all sites information.
        return None, '\t'.join([str(chrom), str(pos), '2', '1', 'all_der_alt_anc'])+'\n', None
    
    #############################
    ## Variant Site Conditions ##
    #############################
    # Else-if the reference allele is the ancestral allele and
    # the alternative allele is not present in modern humans.
    elif (anc_allele.upper() == ref) & (alt != '.') & (hum_gt[0] == '.'):
        # If the ancestral allele call is not already encoded in the info field.
        if 'AA' not in info:
            # Update the info field.
            spline[7] = f'{spline[7]};AA={anc_allele}|||'
        ...
        new_spline.append('0|0:.:.:.:.:.:.:.')
        # Count the number of derived alleles in the archaic.
        arc_dac = int(arc_gt[0] == '1') + int(arc_gt[2] == '1')
        # Return the all sites informtion and the new vcf line.
        return None, '\t'.join([str(chrom), str(pos), f'{arc_dac}', '2', 'hum_anc_ref_anc'])+'\n', '\t'.join(new_spline)+'\n'
    # Else-if the alternative allele is the ancestral allele and
    # the alternative allele is not present in modern humans.
    elif (anc_allele.upper() == alt) & (hum_gt[0] == '.'):
        # If the ancestral allele call is not already encoded in the info field.
        if 'AA' not in info:
            # Update the info field.
            spline[7] = f'{spline[7]};AA={anc_allele}|||'
        ...
        new_spline = [geno.replace('./.:.:.:.:.:.:.:.', '0|0:.:.:.:.:.:.:.') for geno in spline]
        # Append the ancestral genotype.
        new_spline.append('1|1:.:.:.:.:.:.:.')
        # Count the number of derived alleles in the archaic.
        arc_dac = int(arc_gt[0] == '0') + int(arc_gt[2] == '0')
        # Return the all sites informtion and the new vcf line.
        return None, '\t'.join([str(chrom), str(pos), f'{arc_dac}', '3', 'hum_der_alt_anc'])+'\n', '\t'.join(new_spline)+'\n'
    # Else-if the reference allele is the ancestral allele and
    # the alternative allele is present in modern humans.
    elif (anc_allele.upper() == ref) & (alt != '.') & (hum_gt[0] != '.'):
        # If the ancestral allele call is not already encoded in the info field.
        if 'AA' not in info:
            # Update the info field.
        ...
        arc_dac = int(arc_gt[0] == '1') + int(arc_gt[2] == '1')
        # Return the all sites informtion and the new vcf line.
        return None, '\t'.join([str(chrom), str(pos), f'{arc_dac}', '4', 'hum_var_ref_anc'])+'\n', '\t'.join(spline)+'\n'
    # Else-if the alternative allele is the ancestral allele and
    # the alternative allele is present in modern humans.
    elif (anc_allele.upper() == alt) & (hum_gt[0] != '.'):
        # If the ancestral allele call is not already encoded in the info field.
        ...
        arc_dac = int(arc_gt[0] == '0') + int(arc_gt[2] == '0')
        # Return the all sites informtion and the new vcf line.
        return None, '\t'.join([str(chrom), str(pos), f'{arc_dac}', '5', 'hum_var_alt_anc'])+'\n', '\t'.join(spline)+'\n'
...
def filter_init_merge_vcf(vcf, chrom, prefix):
    ...
    all_sites_lines = [] # Format: chrom, pos, arc dac, id, qc.
