...
# Define a function to determine outlier PBS_{MXL:CHB:CEU} clusters in non-overlapping windows.
def pbs_outlier_props_per_window(chrom, window_size):
    ...
    for mxl in ['MXL', 'MXL_NAT', 'MXL_NOT']:
        ...
        results_dicc[mxl] = np.empty((n_winds, 3))
    # For ever window.
    for wind in range(n_winds):
        ...
        for i, mxl in enumerate(['MXL', 'MXL_NAT', 'MXL_NOT']):
            ...
            if n_pbs_per_site > 0:
                ...
                # Update the results.
                results_dicc[mxl][wind, :] = np.array([n_pbs_per_site, n_crit_pbs, n_crit_prop])
            # Else, there are no defined PBS vlaues.
            else:
                # Update the results.
                results_dicc[mxl][wind, :] = np.array([0, 0, np.nan])
    # For every ancestry partition.
    for mxl in ['MXL', 'MXL_NAT', 'MXL_NOT']:
        # Export the results.
        np.savetxt(
            f'../muc19_results/tgp_mod_no_aa/{mxl.lower()}_chb_ceu_pbs_outlier_props_chr{chrom}_{window_size}kb.txt.gz',
            results_dicc[mxl], fmt='%1.15f',
        )
    return
