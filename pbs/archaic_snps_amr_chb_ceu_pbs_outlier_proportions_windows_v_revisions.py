...
def archaic_snps_pbs_outlier_props_per_window(chrom, window_size):
    ...
    results_dicc = {snp_type: {amr: np.empty((n_winds, 3)) for amr in ['PEL', 'CLM', 'PUR']} for snp_type in snp_type_dicc}
    # For ever window.
    for wind in range(n_winds):
        ...
        for amr in ['PEL', 'CLM', 'PUR']:
            ...
            arc_masks = find_arc_alleles(
                ...
                pop_dicc=samp_idx_dicc, ooa_pop=amr, ooa_freq=0.01,
            )
            ...
            for snp_type in arc_masks:
                ...
                if (arc_masks[snp_type].sum() == 0):
                    # Update the results.
                    results_dicc[snp_type][amr][wind, :] = np.array([0, 0, np.nan])
                # Else, there are archaic snps to perform computations on.
                    ...
                    pbs_per_site = calc_pbs_per_site(
                        ...
                        pop_a=samp_idx_dicc[amr], pop_b=samp_idx_dicc['CHB'], pop_c=samp_idx_dicc['CEU'],
                    )
                    ...
                    if n_pbs_per_site > 0:
                        ...
                        # Update the results.
                        results_dicc[snp_type][amr][wind, :] = np.array([n_pbs_per_site, n_crit_pbs, n_crit_prop])
    ...
    for snp_type in results_dicc:
        ...
        for amr in ['PEL', 'CLM', 'PUR']:
            # Export the results.
            np.savetxt(
                f'../muc19_results/tgp_arcs_masked_no_aa/{snp_type_dicc[snp_type]}_{amr.lower()}_chb_ceu_pbs_outlier_props_chr{chrom}_{window_size}kb.txt.gz',
                results_dicc[snp_type][amr], fmt='%1.15f',
            )
    return
