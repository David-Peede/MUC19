...
# Define a function to calculate PBS_{{PEL, CLM, PUR}:CHB:CEU} in non-overlapping windows per archaic snp partition.
def archaic_snps_pbs_windows(chrom, window_size):
        ...
        for i, amr in enumerate(['PEL', 'CLM', 'PUR']):
            ...
            for snp_type in arc_masks:
                ...
                if (arc_masks[snp_type].sum() == 0):
                    # Update the results.
                    pbs_dicc[snp_type][wind, i] = np.nan
    ...
    for snp_type in pbs_dicc:
        # Export the results.
        np.savetxt(
            f'../muc19_results/tgp_arcs_masked_no_aa/{snp_type_dicc[snp_type]}_amr_chb_ceu_pbs_chr{chrom}_{window_size}kb.txt.gz',
