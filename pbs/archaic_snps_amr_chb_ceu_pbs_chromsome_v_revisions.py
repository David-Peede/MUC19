...
def archaic_snps_pbs_chromosome(chrom):
    ...
    for amr in ['PEL', 'CLM', 'PUR']:
        ...
        arc_masks = find_arc_alleles(gt=gt, pop_dicc=samp_idx_dicc, ooa_pop=amr, ooa_freq=0.01)
        # For every archaic snp type.
        for snp_type in snp_type_dicc:
            # Compute PBS.
            amr_arc_pbs = calc_pbs_per_site(
                gt=gt.compress(arc_masks[snp_type], axis=0),
                pop_a=samp_idx_dicc[amr], pop_b=samp_idx_dicc['CHB'], pop_c=samp_idx_dicc['CEU'],
            )
            # Export the results.
            np.savetxt(
                f'../muc19_results/tgp_arcs_masked_no_aa/{snp_type_dicc[snp_type]}_{amr.lower()}_chb_ceu_pbs_chr{chrom}.txt.gz',
                [amr_arc_pbs], fmt='%1.15f',
            )
    return
