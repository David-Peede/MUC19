...
def archaic_pap_windows(chromosome, window_size):
    ...
    pap_configs = [('CHA', 'DEN', 'ALT'), ('VIN', 'DEN', 'ALT')]
    ...
    for config in pap_configs:
        ...
        np.savetxt(
            f'../muc19_results/arcs_masked_no_aa/{t.lower()}_{s1.lower()}_{s2.lower()}_pap_counts_chr{chromosome}_{window_size}kb.txt.gz',
            pap_dicc[config], fmt='%d',
        )
    return

# Calculate pap sites in windows.
archaic_pap_windows(chromosome=int(sys.argv[1]), window_size=int(sys.argv[2]))