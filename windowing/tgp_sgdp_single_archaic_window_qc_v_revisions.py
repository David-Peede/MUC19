...
def windows_qc(window_size, chromosome, prefix):
    ...
    path_dicc = {
        'tgp_den_masked_aa': '../vcf_data/bookkeeping/tgp_den_masked_aa_calls_all_sites',
        'tgp_den_masked_no_aa': '../vcf_data/bookkeeping/tgp_den_masked_no_aa_calls_all_sites',
        'tgp_alt_masked_aa': '../vcf_data/bookkeeping/tgp_alt_masked_aa_calls_all_sites',
        'tgp_alt_masked_no_aa': '../vcf_data/bookkeeping/tgp_alt_masked_no_aa_calls_all_sites',
        'tgp_cha_masked_aa': '../vcf_data/bookkeeping/tgp_cha_masked_aa_calls_all_sites',
        'tgp_cha_masked_no_aa': '../vcf_data/bookkeeping/tgp_cha_masked_no_aa_calls_all_sites',
        'tgp_vin_masked_aa': '../vcf_data/bookkeeping/tgp_vin_masked_aa_calls_all_sites',
        'tgp_vin_masked_no_aa': '../vcf_data/bookkeeping/tgp_vin_masked_no_aa_calls_all_sites',
    }
    ...
    for wind in wind_dicc.keys():
        ...
        if (var_idx.size > 0):
            ...
            results_mat[wind, :] = np.array(
                [wind, start, stop, hum_arc_esl, s, 1], dtype=object,
            )
        # Else-if there are sites that passed qc for this window.
        elif (esl_idx.size > 0):
            # Append the results.
            results_mat[wind, :] = np.array(
                [wind, start, stop, hum_arc_esl, 0, 1], dtype=object,
            )
        # Else.
        else:
            # Append the results.
