...
# Define a function to create psuedo-haploids.
def psuedo_haploidify(gt):
    # Compute the aaf.
    ind_aaf = calc_ind_alt_freqs(gt)
    # Create a mask for the heterzygous sites.
    het_mask = ind_aaf == 0.5
    # Randomly sample an allele per site.
    random_alleles = np.random.randint(2, size=het_mask.sum())
    # Create a psuedo-haploid.
    ind_aaf[het_mask] = random_alleles
    return ind_aaf
...
# Define a function to calculate sequence divergence between psuedo-haplotypes.
def calc_hum_hap_v_arc_phap_diffs(gt):
    # Load the meta data.
    ...
    # Intialize the archaic's psuedo-haplotype.
    arc_phap = psuedo_haploidify(gt.take([2347], axis=1))
    ...
    for samp in range(meta_data.shape[0]):
        ...
        pwd_1 = pwd_per_site(hap_1, arc_phap)
        pwd_2 = pwd_per_site(hap_2, arc_phap)
        # Update dictionaries.
        pwd_dicc['hap_1'] = np.append(pwd_dicc['hap_1'], np.nansum(pwd_1))
        pwd_dicc['hap_2'] = np.append(pwd_dicc['hap_2'], np.nansum(pwd_2))
    return pwd_dicc

# Define a function to calculate psuedo-haplotype divergence in non-overlapping windows.
def tgp_hap_v_arc_phap_diffs_windows(chromosome, window_size, archaic):
    ...
    for wind in range(n_windows):
        ...
        # Compute psuedo-haplotype distances.
        pw_diffs = calc_hum_hap_v_arc_phap_diffs(allel.GenotypeArray(callset[wind_loc]))
        # For every haplotype.
    ...
    for hap in ['hap_1', 'hap_2']:
        ...
        np.savetxt(
            f'../muc19_results/tgp_{archaic.lower()}_masked_no_aa/{archaic.lower()}_{hap}_psuedo_hap_diffs_chr{chromosome}_{window_size}kb.txt.gz',
            pw_diffs_dicc[hap], fmt='%1.15f',
        )
    return

# Calculate haplotype divergence in non-overlapping windows.
tgp_hap_v_arc_phap_diffs_windows(
